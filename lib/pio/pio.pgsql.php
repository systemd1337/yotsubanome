 ...
 *
 * @package PMCLibrary
 * @version $Id: pio.pgsql.php 671 2008-09-20 05:54:15Z scribe $
...
class PIOpgsql{
	...
	function dbConnect($connStr){
		...
		// 示例： pgsql://yotsubanome:pass@localhost/yotsubanome/img_loog/
	...
	PRIMARY KEY (\"no\"));"; // PIO Structure V4
			...
			if($isAddInitData) $this->addPost(0, 0, '', '', 0, '', 0, 0, '', '', 0, 0, '', '08/11/08(土) 10:24:04', '【スパーキー（④ ^ヮ^）】', '', $this->ENV['NOTITLE'], $this->ENV['NOCOMMENT'], ''); // 追加一筆新資料
			$this->dbCommit();
		}
	}
	...
	$line[7].','.$line[8].',\''.pg_escape_string($line[9]).','.pg_escape_string($line[10]).'\','.$line[11].','.$line[12].',\''.
	...
	function dbExport(){
		...
		$line = $this->_pgsql_call('SELECT no,resto,root,md5chksum,category,tim,ext,imgw,imgh,imgsize,filename,tw,th,pwd,now,name,email,sub,com,host,status FROM '.$this->tablename.' ORDER BY no DESC',
