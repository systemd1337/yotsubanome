 ...
 *
 * @package PMCLibrary
 * @version $Id: pio.log.php 671 2008-09-20 05:54:15Z scribe $
...
class PIOlog{
	...
	function _AnalysisLogs($line){
		$tline = array();
		list($tline['no'], $tline['resto'], $tline['md5chksum'], $tline['category'], $tline['tim'], $tline['ext'], $tline['imgw'], $tline['imgh'], $tline['imgsize'], $tline['filename'], $tline['tw'], $tline['th'], $tline['pwd'], $tline['now'], $tline['name'], $tline['email'], $tline['sub'], $tline['com'], $tline['host'], $tline['status']) = explode(',', $line);
	...
	function dbInit(){
		...
		foreach($chkfile as $value){
			if(!is_file($value)){ // 檔案不存在
				...
				if($value==$this->logfile) fwrite($fp, '0,0,,,0,,0,0,,,0,0,,08/11/08(土) 10:24:04,【スパーキー（④ ^ヮ^）】,,'.$this->ENV['NOTITLE'].','.$this->ENV['NOCOMMENT'].',,,'); // PIO Structure V4
				if($value==$this->treefile) fwrite($fp, '0');
				if($value==$this->porderfile) fwrite($fp, '0');
				fclose($fp);
				unset($fp);
				@chmod($value, 0666);
			}
	...
	function addPost($no, $resto, $md5chksum, $category, $tim, $ext, $imgw, $imgh, $imgsize, $filename, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $age=false, $status='') {
		...
		$tline = array($no, $resto, $md5chksum, $category, $tim, $ext, $imgw, $imgh, $imgsize, $filename, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $status);
