 ...
 *
 * @package PMCLibrary
 * @version $Id: pio.sqlite3.php 658 2008-08-06 15:59:17Z scribe $
 * @date $Date: 2008-08-06 23:59:17 +0800 (星期三, 06 八月 2008) $
 */
...
class PIOsqlite3{
	...
	/* PIO模組版本 */
	function pioVersion(){
		return '0.6 (v20100404)';
	}

	/* 處理連線字串/連接 */
	public function dbConnect($connStr){
		// 格式： sqlite3://資料庫檔案之位置/資料表/
		// 示例： sqlite://yotsubanome.db/img_loog/
		// 　　　 sqlite3://:memory:/img_loog/
	...
	"status" VARCHAR(255)  NOT NULL
			...
			if($isAddInitData) $result .= 'INSERT INTO '.$this->tablename.' (no,resto,root,time,md5chksum,category,tim,ext,imgw,imgh,imgsize,filename,tw,th,pwd,now,name,email,sub,com,host,status) VALUES (0, 0, datetime("now"), 1111111111, "", "", 1111111111111, "", 0, 0, "", "", 0, 0, "", "08/11/08(土)10:24:04", "【スパーキー（④ ^ヮ^）】", "", "'.$this->ENV['NOTITLE'].'", "'.$this->ENV['NOCOMMENT'].'", "", "");';
	...
	public function dbImport($data){
		...
		$SQL = 'INSERT INTO '.$this->tablename.' (no,resto,root,time,md5chksum,category,tim,ext,imgw,imgh,imgsize,filename,tw,th,pwd,now,name,email,sub,com,host,status) VALUES '
				.'(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
		...
		for($i = 0; $i < $data_count; $i++){
			...
			$PDOStmt->bindValue(12, $line[10], PDO::PARAM_STR);
			$PDOStmt->bindValue(13, $line[11], PDO::PARAM_INT);
			$PDOStmt->bindValue(14, $line[12], PDO::PARAM_INT);
			...
			$PDOStmt->bindValue(22, $line[20], PDO::PARAM_STR);
	...
	public function dbExport(){
		...
		$line = $this->con->query('SELECT no,resto,root,md5chksum,category,tim,ext,imgw,imgh,imgsize,filename,tw,th,pwd,now,name,email,sub,com,host,status FROM '.$this->tablename.' ORDER BY no DESC');
		$data = '';
		$replaceComma = create_function('$txt', 'return str_replace(",", "&#44;", $txt);');
	...
	/* 新增文章/討論串 */
	public function addPost($no, $resto, $md5chksum, $category, $tim, $ext, $imgw, $imgh, $imgsize, $filename, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $age=false, $status='') {
		if(!$this->prepared) $this->dbPrepare();
		...
		$SQL = 'INSERT INTO '.$this->tablename.' (resto,root,time,md5chksum,category,tim,ext,imgw,imgh,imgsize,filename,tw,th,pwd,now,name,email,sub,com,host,status) VALUES '
				.'(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
		$PDOStmt = $this->con->prepare($SQL);
		...
		$PDOStmt->bindValue(11, $filename, PDO::PARAM_STR);
		$PDOStmt->bindValue(12, $tw, PDO::PARAM_INT);
		$PDOStmt->bindValue(13, $th, PDO::PARAM_INT);
		$PDOStmt->bindValue(14, $pwd, PDO::PARAM_STR);
		$PDOStmt->bindValue(15, $now, PDO::PARAM_STR);
		$PDOStmt->bindValue(16, $name, PDO::PARAM_STR);
		$PDOStmt->bindValue(17, $email, PDO::PARAM_STR);
		$PDOStmt->bindValue(18, $sub, PDO::PARAM_STR);
		$PDOStmt->bindValue(19, $com, PDO::PARAM_STR);
		$PDOStmt->bindValue(20, $host, PDO::PARAM_STR);
		$PDOStmt->bindValue(21, $status, PDO::PARAM_STR);
		$PDOStmt->execute() or $this->_error_handler('Insert a new post failed', __LINE__);
	}
	...
	public function updatePost($no, $newValues){
		...
		$no = intval($no);
		$chk = array('resto', 'md5chksum', 'category', 'tim', 'ext', 'imgw', 'imgh', 'imgsize', 'filename', 'tw', 'th', 'pwd', 'now', 'name', 'email', 'sub', 'com', 'host', 'status');
		foreach($chk as $c){
