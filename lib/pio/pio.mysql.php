 ...
 *
 * @package PMCLibrary
 * @version $Id: pio.mysql.php 671 2008-09-20 05:54:15Z scribe $
...
class PIOmysql{
	...
	function dbConnect($connStr){
		...
		// 示例： mysql://yotsubanome:pass@localhost/yotsubanome/img_loog/
	...
	COMMENT = 'PIO Structure V4'";
			...
			// 追加一筆新資料
			if($isAddInitData) $this->addPost(0, 0, '', '', 0, '', 0, 0, '', '', 0, 0, '', '08/11/08(土) 10:24:04', '【スパーキー（④ ^ヮ^）】', '', $this->ENV['NOTITLE'], $this->ENV['NOCOMMENT'], ''); // 追加一筆新資料料
			$this->dbCommit();
		}
	}
	...
	$line[7].','.$line[8].',\''.mysql_real_escape_string($line[9], $this->con).','.mysql_real_escape_string($line[10], $this->con).'\','.$line[11].','.$line[12].',\''.
	...
	mysql_real_escape_string($line[20], $this->con).'\')';
	...
	function dbExport(){
		...
		$line = $this->_mysql_call('SELECT no,resto,root,md5chksum,category,tim,ext,imgw,imgh,imgsize,filename,tw,th,pwd,now,name,email,sub,com,host,status FROM '.$this->tablename.' ORDER BY no DESC',
	...
	/* 新增文章/討論串 */
	function addPost($no, $resto, $md5chksum, $category, $tim, $ext, $imgw, $imgh, $imgsize, $filename, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $age=false, $status='') {
		if(!$this->prepared) $this->dbPrepare();
